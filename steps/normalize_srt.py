...
1. ATAMALAR ALMASHTIRILADI (full_terms.json). Tarjima qilinmaydigan inglizcha
...
def normalize_srt(
    ...
    if replacements_path is None and ask_replacements:
        replacements_path = ask_optional_path(
            "Atamalar JSON fayli manzilini kiriting (/path/to/docker/full_terms.json)"
        )
    ...
    normalized: list[Cue] = []
    for cue in cues:
        # 1) full_terms.json qoidalari, 2) undan keyin uztts.normalize.
...
def replace_terms(text: str, pattern: re.Pattern[str] | None, replacements: dict[str, str]) -> str:
    """Matndagi atamalarni full_terms.json bo'yicha almashtirish.
